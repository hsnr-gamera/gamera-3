...
def make_test(inner):
   ...
   def test():
      for type, value in [(ONEBIT, xrange(0, (2 ** 16) - 1)),
                          (GREYSCALE, xrange(0, (2 ** 8) - 1)),
                          (FLOAT, [float(x) for x in xrange(-5, 5)]),
                          (COMPLEX, [complex(x) for x in xrange(-5, 5)]),
                          (RGB, _generate_rgb()),
                          (GREY16, xrange(0, (2 ** 16) - 1))]:
...
def _test_image_constructors(type, value, storage):
   ...
   image = Image(25, 25, 50, 50, type, storage)
   ...
   assert image.dimensions == Dimensions(50, 50)
   ...
   image4 = Image(Point(25, 25), Dimensions(50, 50), type, storage)
   assert Rect(image) == Rect(image4)
   image5 = Image(Rect(25, 25, 50, 50), type, storage)
   assert Rect(image) == Rect(image5)
   py.test.raises(TypeError, _fail1)

test_image_contructors = make_test(_test_image_constructors)
...
def _test_subimage(type, value, storage):
   ...
   image = Image(25, 25, 50, 50, type, storage)
   image2 = image.subimage(30, 30, 40, 40)
