...
class OptimizerFrame(wxFrame):
   ...
   def open_cb(self, evt):
      ...
      if filename:
         ...
         try:
            print "1"
            glyphs = gamera.gamera_xml.glyphs_with_features_from_xml(
                self.filename)
            print "2"
         except Exception, e:
            wxEndBusyCursor()
   ...
   def update_status(self):
      if not self.running:
         ...
         self.status.initial_rate_display.SetValue(
             str(self.classifier.ga_initial * 100.0))
         self.status.current_rate_display.SetValue(
             str(self.classifier.ga_best * 100.0))
         self.status.generation_display.SetValue(
             str(self.classifier.ga_generation))
