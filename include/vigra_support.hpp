...
namespace Gamera {
  ...
  */
  ...
  public:
	...
	if (m_accessor(tmpi) == m_label)
	  if (tmp) {
	    m_accessor.set(tmpi, 0);
	  } else {
	    m_accessor.set(tmpi, m_label);
	  }
    }
    OneBitPixel m_label;
  ...
  */
  ...
  public:
    ...
    { 
      if (value)
	m_accessor.set(i + diff, 0);
      else
	m_accessor.set(i + diff, 1);
    }
    ImageAccessor<value_type> m_accessor;
  };
