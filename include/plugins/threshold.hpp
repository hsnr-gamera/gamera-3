...
template<class T, class U>
void parker_threshold_get_neighbors(const T& image, size_t y, size_t x, U* p) {
  ...
  p[0] = (image.get(y_before, x));
  p[1] = (image.get(y_before, x_after));
  p[2] = (image.get(y, x_after));
  p[3] = (image.get(y_after, x_after));
  p[4] = (image.get(y_after, x));
  p[5] = (image.get(y_after, x_before));
  p[6] = (image.get(y, x_before));
  p[7] = (image.get(y_before, x_before));
}
...
template<class T>
void parker_threshold_reg_mean_dev(const T& in, FloatImageView& dev_view, FloatImageView& mean_view) {
  typedef typename T::value_type pixel_type;
  size_t xend = -1;
  for (size_t rx = 0; rx < dev.ncols(); ++rx) {
    size_t xstart = xend + 1;
    xend = (in.ncols() * rx) / dev.ncols();
    size_t yend = -1;
    for (size_t ry = 0; ry < dev.nrows(); ++ry) {
      size_t starty = yend + 1;
	...
	for (y = ystart; y <= yend; ++y) {
      ...
      mean_view.set(y, x, mean);
      double variance = (sum2 * 1.0 / n) - mean * mean;
      dev_view.set(y, x, sqrt(variance));
    }
  }
}

template<class T>
Image* parker_threshold(const T &m, int storage_format, size_t region_size, double k, double mean_lim,
			double sdev_lim, bool abs_value) {
  size_t regions_x = m.ncols() / region_size;
  size_t regions_y = m.nrows() / region_size;
  typedef FloatImageData histogram_data_type;
  typedef FloatImageView histogram_type;
  histogram_data_type min_grad_data(m.nrows(), m.ncols());
  histogram_type min_grad(min_grad_data);
  histogram_data_type qdev_data(m.nrows(), m.ncols());
  histogram_type qdev(qdev_data);
  histogram_data_type qmean_data(m.nrows(), m.ncols());
  histogram_type qmean(qmean_grad_data);
  histogram_data_type qdev1_data(regions_y, regions_x);
  histogram_type qdev1(qdev1_grad_data);
  histogram_data_type qmean1_data(regions_y, regions_x);
  histogram_type qmean1(qmean1_grad_data);
  histogram_data_type qdev2_data(regions_y, regions_x);
  histogram_type qdev2(qdev2_grad_data);
  histogram_data_type qmean2_data(regions_y, regions_x);
  histogram_type qmean2(qmean2_grad_data);
 
  if (abs_value)
    parker_threshold_min_grad(m, min_grad);
  else
    parker_threshold_min_grad_old(m, min_grad);

  parker_threshold_reg_mean_dev(min_grad, qdev1, qmean1);
  parker_threshold_reg_smooth(qdev1, qdev2);
  parker_threshold_reg_smooth(qmean1, qmean2);
