//
// Copyright (C) 2010 Oliver Christen
//
...
void DelaunayTree::output(std::multimap<int, int> *nhmap) {
...
void Triangle::output(std::multimap<int,int> *nhmap) {
	TriangleList* l;
	...
	if(neighbors[0]->number != number) {
		if(!flag.isInfinite()) {
			...
			if( (vertices[1]->getLabel() >= 0 && vertices[2]->getLabel() >= 0)
			   && (vertices[1]->getLabel() != vertices[2]->getLabel())  ) {
				//std::cout << vertices[1]->getX() << " " << vertices[1]->getY() << " " << vertices[2]->getX() << " " << vertices[2]->getY() << std::endl;
				nhmap->insert( std::pair<int, int>(vertices[1]->getLabel(), vertices[2]->getLabel()) );
			}
	...
	if(neighbors[1]->number != number) {
        if (!flag.isInfinite()) {
        	...
        	if( (vertices[2]->getLabel() >= 0 && vertices[0]->getLabel() >= 0)
			   && (vertices[2]->getLabel() != vertices[0]->getLabel()) ) {
				//std::cout << vertices[2]->getX() << " " << vertices[2]->getY() << " " << vertices[0]->getX() << " " << vertices[0]->getY() << std::endl;
				nhmap->insert( std::pair<int, int>(vertices[2]->getLabel(), vertices[0]->getLabel()) );
			}
        	
        } else if (( flag.isInfinite() == 1) && (flag.isLastFinite())) {
	...
	if(neighbors[2]->number != number) {
        if(!flag.isInfinite()) {
        	...
        	if( (vertices[0]->getLabel() >= 0 && vertices[1]->getLabel() >= 0)
			   && (vertices[0]->getLabel() != vertices[1]->getLabel()) ) {
				//std::cout << vertices[0]->getX() << " " << vertices[0]->getY() << " " << vertices[1]->getX() << " " << vertices[1]->getY() << std::endl;
				nhmap->insert( std::pair<int, int>(vertices[0]->getLabel(), vertices[1]->getLabel()) );
			}
